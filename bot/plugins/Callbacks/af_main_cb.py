...
async def cb_navg(bot, update: CallbackQuery):
    ...
    if ((index_val + 1 )== max_pages) or ((index_val + 1) == len(results)): # Max Pages
        ...
        temp_results.append([
            InlineKeyboardButton("⚜️Click Here To Back⚜️", callback_data=f"navigate({index_val}|back|{query})"),
            InlineKeyboardButton("⚜️Click Here To Next⚜️", callback_data=f"navigate({index_val}|next|{query})")
        ])
...
async def callback_data(bot, update: CallbackQuery):
    ...
    if query_data == "start":
        buttons = [[
            InlineKeyboardButton('My Dev 👨‍🔬', url='https://t.me/crizzbizzyt'),
            InlineKeyboardButton('Source Code 🧾', url ='https://github.com/MOVIESCLUBmy/Auto-Filter-Bot-V2')
        ],[
            InlineKeyboardButton('Support 🛠', url='https://t.me/MOVIES_CLUB_GROUP_2')
        ],[
